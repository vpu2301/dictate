...
_UNITS: Final[dict[str, str]] = {
    "мг": "мг", "міліграм": "мг", "міліграми": "мг", "міліграмів": "мг",
    "мл": "мл", "мілілітр": "мл", "мілілітри": "мл", "мілілітрів": "мл",
    "см": "см", "сантиметр": "см", "сантиметри": "см", "сантиметрів": "см",
    "мм": "мм",
    "м": "м", "метр": "м", "метрів": "м",
    "кг": "кг", "кілограм": "кг", "кілограми": "кг", "кілограмів": "кг",
    "г": "г", "грам": "г", "грами": "г", "грамів": "г",
    "л": "л", "літр": "л", "літрів": "л",
    "мкг": "мкг", "мікрограм": "мкг", "мікрограмів": "мкг",
}
...
_DIGITS_UK: Final[dict[str, int]] = {
    "нуль": 0, "один": 1, "одна": 1, "одну": 1, "одного": 1, "одної": 1,
    "два": 2, "дві": 2, "двох": 2,
    "три": 3, "трьох": 3, "чотири": 4, "чотирьох": 4,
    "п'ять": 5, "п'яти": 5, "пять": 5, "пяти": 5,
    "шість": 6, "шести": 6, "сім": 7, "семи": 7,
    "вісім": 8, "восьми": 8, "дев'ять": 9, "дев'яти": 9, "девять": 9, "девяти": 9,
    "десять": 10, "десяти": 10,
    ...
    "тридцять": 30, "тридцяти": 30,
    "сорок": 40, "сорока": 40,
    "п'ятдесят": 50, "п'ятдесяти": 50, "пятдесят": 50, "пятдесяти": 50,
    "шістдесят": 60, "шістдесяти": 60, "сімдесят": 70, "сімдесяти": 70,
    "вісімдесят": 80, "вісімдесяти": 80, "дев'яносто": 90, "дев'яноста": 90,
    "девяносто": 90, "девяноста": 90,
    "сто": 100, "ста": 100, "двісті": 200, "двохсот": 200, "триста": 300, "трьохсот": 300,
    "чотириста": 400, "чотирьохсот": 400, "п'ятсот": 500, "п'ятисот": 500,
    "шістсот": 600, "шестисот": 600, "сімсот": 700, "семисот": 700,
    "вісімсот": 800, "восьмисот": 800, "дев'ятсот": 900, "дев'ятисот": 900,
    "тисяча": 1000, "тисячі": 1000, "тисяч": 1000,
}

_HOUR_SPELLED = {
    "восьму": 8, "сьому": 7, "шосту": 6, "дев'яту": 9, "десяту": 10,
    "одинадцяту": 11, "дванадцяту": 12, "першу": 1, "другу": 2, "третю": 3,
    "четверту": 4, "п'яту": 5,
}
...
def normalize_uk(text: str, *, decimal_separator: str, bp_separator: str) -> str:
    ...
    while i < n:
        ...
        if (
            ...
            out.append(f"{v1} {('раз' if v1 == 1 else 'рази')}/{raw[i + c1 + 2].lower()[:-1] + 'у' if v1 == 1 else 'добу' if raw[i + c1 + 2].lower() == 'добу' else 'день'}")
            # Simpler canonical: "X разів/добу"
            out[-1] = f"{v1} разів/добу" if raw[i + c1 + 2].lower() == "добу" else f"{v1} разів/день"
            i += c1 + 3
            continue
