...
- **WER eval on macOS:** `make wer-eval-corpus` (or a bare `uv run python scripts/eval/run_wer.py --corpus eval/corpus`) runs locally with no GPU. `run_wer.py` auto-selects `cpu`/`int8`/`tiny` on macOS (explicit `MD_ASR_*` win), and `faster-whisper` — excluded from asr-worker's runtime deps on Darwin — is pulled into the dev venv via the macOS-gated `[dependency-groups] dev` in the root `pyproject.toml`. Needs `ffmpeg`. Numbers are plumbing-only (CPU/`tiny` + 8 placeholder fixtures → WER 1.0); the real gate is `large-v3` on the Linux/GPU rig. See `docs/eval/wer-methodology.md`.
